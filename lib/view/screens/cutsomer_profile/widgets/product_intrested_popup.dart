...
class _ProductInterestedPopupState extends State<ProductInterestedPopup> {
  ...
  void _initializeProducts() {
    if (widget.initialProducts != null) {
      _products = widget.initialProducts!.map((product) {
        return ProductItem(
          productId: product['product_id'] ?? '',
          productName: product['product_name'] ?? '',
          offers: (product['offers'] as List?)?.map((offer) {
                return OfferItem(
                  offerPrice: offer['offer_price']?.toDouble() ?? 0.0,
                  demandingPrice: offer['demanding_price']?.toDouble() ?? 0.0,
                  description: offer['description'] ?? '',
                  updatedBy: offer['updated_by'] ?? '',
                  status: offer['status'] ?? 'active',
                  uploadedAt: offer['uploaded_at'] ?? '',
                );
              }).toList() ??
              [],
        );
